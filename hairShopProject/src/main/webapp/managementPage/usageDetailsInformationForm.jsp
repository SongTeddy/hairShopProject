				...
				<div id="myInfoModal" class="infoModal">
					<div class="infoModal-content">
						...
						<div class="input-container">
							<span style="font-weight: bold; margin-right: 50px;">리뷰 제목</span>
							<input type="text" class="designerSubject" style="margin-top: 10px;" size="34" placeholder="리뷰 제목 입력">
							<div class="designerSubjectDiv"></div>
						</div>
						<div class="input-container" style="margin-top: 10px;">
							<span style="font-weight: bold; margin-right: 50px;">리뷰 내용</span>
							<textarea class="designerContent" cols="36" rows="8" style="overflow-y:scroll; resize:none; vertical-align: middle;" placeholder="리뷰  내용 입력"></textarea>
							<div class="designerContentDiv"></div>
						</div>
						<div class="input-container hairShopStarRev">
							<span style="font-weight: bold; margin-right: 95px;">미용실 별점</span>
						...
						<div class="input-container designerStarRev">
							<span style="font-weight: bold; margin-right: 79px;">디자이너 별점</span>
						...
						<div class="button-container" style="margin-top: 50px;">
							<input type="button" class="reviewWriteBtn" style="width: 70px; margin-right: 10px;" value="리뷰 작성">
							<input type="reset" style="width: 70px;" value="다시 작성">
						</div>
					</div>
				</div>
			</tbody>
...
$(document).ready(function(){
		...
		success : function(data){
			$.each(data.list, function(index, items){
				...
				if(items.DIDORDIDNOT==0){
					...
					reviewButton.innerHTML='<input type="button" id='+items.SEQ+' onclick="reviewPop(\''+theday+'\')" class="serviceCheck" value="Review">'
				}
	...
	$('.reviewWriteBtn').on('click',function(){
		alert($('.usageDetailsInformationEmail').val()+", "+$('.hiddenTheDay').val()+", "+$('.hairShopStar').val()+", "+$('.designerStar').val());
		...
		if($('.designerSubject').val()==""){
			...
			$.ajax({
				...
				data : {'email':$('.usageDetailsInformationEmail').val(), 'theday':#('.hiddenTheDay').val(), 'hairShopStar':$('.hairShopStar').val(), 'designerStar':$('.designerStar').val(), 'designerSubject':$('#designerSubject').val(), 'designerContent':$('#designerContent').val()},
				dataType : 'text',
				success : function(data){
					alert(data);	
				},
